package com.lenovoved.android.service
...
import com.lenovoved.android.cards.GenericCard
import com.lenovoved.android.cards.MediaCard
import com.lenovoved.android.cards.NavigationCard
import com.lenovoved.android.cards.PaymentCard
import com.lenovoved.android.cards.SportsCard
import com.lenovoved.android.cards.fallbackClickIntent
import com.lenovoved.android.island.PlaygroundService
import com.lenovoved.android.log.CastLog
import com.lenovoved.android.sports.SportsFeed
import com.lenovoved.android.sports.SportsParser
...
class NotificationCastListener : NotificationListenerService() {
    ...
    companion object {
        ...
        /**
         * Notifies the listener when media casting setting changes.
         * If disabled, cancels any active media cards immediately from the island.
         */
        fun onMediaSettingsChanged(context: Context, enabled: Boolean) {
            if (!enabled) {
                val mediaIds = IconCache.activeControllers.keys.toList()
                for (id in mediaIds) {
                    IconCache.clear(id)
                    runCatching {
        ...
        /**
         * Notifies the listener when a category casting setting changes ("normal", "messenger", "navigation").
         * If disabled, cancels any active cards of that category immediately.
         */
        fun onCategorySettingsChanged(context: Context, category: String, enabled: Boolean) {
            if (!enabled) {
                val activeIds = PlaygroundService.activeCardIds.toList()
                for (id in activeIds) {
                    val cat = PlaygroundService.cardCategoryMap[id] ?: "normal"
                    val matches = when (category) {
                        "media" -> IconCache.activeControllers.containsKey(id)
                        "messenger" -> cat == "messenger"
        ...
        /**
         * Notifies the listener when notification casting setting changes.
         */
        fun onNotificationsSettingsChanged(context: Context, enabled: Boolean) {
            if (!enabled) {
                val prefs = context.getSharedPreferences(PREFS, 0)
                val castMedia = prefs.getBoolean("cast_media_sessions", false)
                val activeIds = PlaygroundService.activeCardIds.toList()
                for (id in activeIds) {
                    val isMediaId = IconCache.activeControllers.containsKey(id)
                    if (!isMediaId || !castMedia) {
                        IconCache.clear(id)
                        runCatching {
                            context.startService(
                                Intent(context, PlaygroundService::class.java)
        ...
        /**
         * Notifies the listener when app filter changes.
         * Immediately clears active cards for excluded apps from the island and reloads.
         */
        fun onAppFilterChanged(context: Context, pkg: String? = null, enabled: Boolean? = null) {
            val prefs = context.getSharedPreferences(PREFS, 0)
            val ignored = prefs.getStringSet("cast_ignored_apps", emptySet()).orEmpty()

            // 1. Cancel active media controllers if their package is now ignored
            val mediaIds = IconCache.activeControllers.filter { entry ->
                val p = entry.value.packageName
                p in ignored || (pkg != null && p == pkg && enabled == false)
            }.keys.toList()
            for (id in mediaIds) {
                IconCache.clear(id)
                runCatching {
            ...
            // 2. Cancel package cards in PlaygroundService
            if (pkg != null && enabled == false) {
                runCatching {
                    context.startService(
                        Intent(context, PlaygroundService::class.java)
                            .setAction(PlaygroundService.ACTION_CANCEL_PACKAGE)
            ...
            // 3. Clean up active listener notifications matching ignored apps
            instance?.let { listener ->
                val active = listener.activeNotifications
                active?.forEach { sbn ->
                    val shouldCancel = sbn.packageName in ignored || (pkg != null && sbn.packageName == pkg && enabled == false)
                    if (shouldCancel) {
                        val id = IconCache.castIdFor(sbn)
                        IconCache.clear(id)
                        listener.lastOutcome.remove(sbn.key)
                        listener.lastScorePostTime.remove(sbn.key)
                        listener.lastCastPostTime.remove(sbn.key)
                        runCatching {
                            context.startService(
                                Intent(context, PlaygroundService::class.java)
                                    .setAction(PlaygroundService.ACTION_CANCEL)
        ...
        /** Known messenger packages. */
        private val MESSENGER_APPS = setOf(
    ...
    /** Last media state key ("$playing|$track") per cast ID to prevent infinite auto-dismiss loops for paused players. */
    private val lastMediaState = ConcurrentHashMap<Int, String>()
    ...
    private val pollRunnable = object : Runnable {
        override fun run() {
            try {
                ...
                val castNormal = prefs.getBoolean("cast_normal_notifications", false) || prefs.getBoolean("cast_notifications", false)
                val castMedia = prefs.getBoolean("cast_media_sessions", false)
                activeNotifications?.forEach { sbn ->
                    if (!isAppEnabled(prefs, sbn.packageName)) return@forEach
                    ...
                    if (token != null) {
                        if (castMedia) {
                            val controller = MediaController(this@NotificationCastListener, token)
                            if (isAppEnabled(prefs, controller.packageName)) {
                                val id = IconCache.castIdFor(sbn)
                                val playbackStateValue = controller.playbackState?.state
                                val playing = when (playbackStateValue) {
                                ...
                                val track = md?.getString(android.media.MediaMetadata.METADATA_KEY_TITLE)?.trim().orEmpty()
                                val stateKey = "$playing|$track"
                                val lastState = lastMediaState[id]

                                if (playing) {
                                    MediaCard.post(applicationContext, sbn, controller)
                                    lastMediaState[id] = stateKey
                                } else if (id in PlaygroundService.activeCardIds) {
                                    // Only post paused update if the state changed (e.g. just paused)
    ...
    fun reload() {
        ...
        val castMedia = prefs.getBoolean("cast_media_sessions", false)
        val castNormal = prefs.getBoolean("cast_normal_notifications", false) || prefs.getBoolean("cast_notifications", false)
        val castMessenger = prefs.getBoolean("cast_messenger_notifications", false)
        val castNav = prefs.getBoolean("cast_nav_notifications", false)
        val castSports = prefs.getBoolean("cast_sports_enabled", false)
        val castPayments = prefs.getBoolean("cast_payments_enabled", false)
        if (!castMedia && !castNormal && !castMessenger && !castNav && !castSports && !castPayments) return
        activeNotifications?.forEach { sbn ->
            if (sbn.packageName != packageName && isAppEnabled(prefs, sbn.packageName)) {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification) {
        ...
        val castSports = prefs.getBoolean("cast_sports_enabled", false)
        val castPayments = prefs.getBoolean("cast_payments_enabled", false)
        if (!castMedia && !castNormal && !castMessenger && !castNav && !castSports && !castPayments) {
            log(sbn, "skipped — all casting toggles are off", false)
            return
        }
        ...
        val isMediaStyle = template == "android.app.Notification\$MediaStyle" ||
            ...
            token != null ||
            n.category == NotificationCompat.CATEGORY_TRANSPORT
        val fallbackController = if (!isCall) findMediaControllerFallback(sbn.packageName) else null
        val isMedia = (isMediaStyle || fallbackController != null) && !isCall

        // 1. Media sessions get the dedicated media path.
        if (isMedia) {
            if (!castMedia) {
                log(sbn, "skipped — media player casting is off", false)
                return
            }
            val controller = token?.let { MediaController(this, it) } ?: fallbackController
            if (controller != null) {
                val id = IconCache.castIdFor(sbn)
                val playbackStateValue = controller.playbackState?.state
                val playing = when (playbackStateValue) {
                ...
                if (!playing && id !in PlaygroundService.activeCardIds && lastMediaState[id] == stateKey) {
                    log(sbn, "skipped — paused media already auto-dismissed", false)
                    return
                }
                ...
                log(sbn, "skipped — media notification without controller", false)
                return
            }
        }
        ...
        if (isNavigation(sbn, isOngoing)) {
            if (!castNav) {
                log(sbn, "skipped — navigator notifications are off", false)
                return
            }
        ...
        // 3. Messengers and chats
        if (isMessenger(sbn)) {
            if (!castMessenger) {
                log(sbn, "skipped — messenger notifications are off", false)
                return
            }
            if (lastCastPostTime.put(sbn.key, sbn.postTime) == sbn.postTime) {
                log(sbn, "skipped — duplicate delivery, unchanged", false)
                return
            }
        ...
        // 4. Normal notifications (system, downloads, calls, progress, sports, payments)
        if (!castNormal) {
            log(sbn, "skipped — normal notifications are off", false)
            return
        }

        // Score apps
        if (sbn.packageName in SCORE_APPS && prefs.getBoolean("cast_sports_enabled", false)) {
        ...
        // Payments
        if (prefs.getBoolean("cast_payments_enabled", false)) {
        ...
        if (!isLive && !wasLive) {
            val ignoreSilent = prefs.getBoolean("cast_ignore_silent", false)
        ...
        if (kind == "normal" && lastCastPostTime.put(sbn.key, sbn.postTime) == sbn.postTime) {
            log(sbn, "skipped — duplicate delivery, unchanged", false); return
        }
    ...
    /**
     * Whether [sbn] is an active turn-by-turn navigation notification.
     */
    private fun isNavigation(sbn: StatusBarNotification, isOngoing: Boolean): Boolean {
        if (!isOngoing && sbn.notification.category != Notification.CATEGORY_NAVIGATION) return false
        val extras = sbn.notification.extras
    ...
    /**
     * Whether [sbn] is a messaging or chat notification.
     */
    private fun isMessenger(sbn: StatusBarNotification): Boolean {
        val n = sbn.notification
        val extras = n.extras
        val template = extras.getString(NotificationCompat.EXTRA_TEMPLATE)
        if (template == "android.app.Notification\$MessagingStyle" ||
            template == "androidx.core.app.NotificationCompat\$MessagingStyle"
        ) return true
        if (n.category == NotificationCompat.CATEGORY_MESSAGE ||
            n.category == NotificationCompat.CATEGORY_SOCIAL
        ) return true
        if (sbn.packageName in MESSENGER_APPS) return true
        if (extras.containsKey(NotificationCompat.EXTRA_MESSAGING_STYLE_USER) ||
            extras.containsKey(NotificationCompat.EXTRA_CONVERSATION_TITLE) ||
            extras.containsKey(NotificationCompat.EXTRA_MESSAGES)
        ) return true
        return false
    }
