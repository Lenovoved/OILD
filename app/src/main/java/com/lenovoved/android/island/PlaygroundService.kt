package com.lenovoved.android.island
...
import com.lenovoved.android.service.IconCache
...
class PlaygroundService : Service() {
    ...
    companion object {
        ...
        const val ORIGIN_CHANNEL_ID = "lenovoved_island_hi"
    ...
    /** Matches the listener's own gate — any card source keeps the service worth running. */
    private fun isCastingEnabled(): Boolean = getSharedPreferences(PREFS_NAME, 0).let {
        it.getBoolean("cast_media_sessions", false) ||
        it.getBoolean("cast_normal_notifications", false) ||
    ...
    private fun ensureForeground() {
        ...
        val n = NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Пространство Origin")
            .setContentText("Служба трансляции на остров активна")
    ...
    private fun readAndPost(intent: Intent) {
        ...
        if (!sourcePkg.isNullOrBlank()) {
            cardPackageMap[id] = sourcePkg
        }
        val category = intent.getStringExtra("category") ?: "normal"
        cardCategoryMap[id] = category
        ...
        val prefAod = globalPrefs.getBoolean("cast_display_aod", false)
        val prefLockscreen = globalPrefs.getBoolean("cast_lockscreen_live_card", false)
        val prefNotification = globalPrefs.getBoolean("cast_display_notification", false)
        val prefWidget = globalPrefs.getBoolean("cast_display_widget", false)
        val prefVSuggestion = globalPrefs.getBoolean("cast_display_v_suggestion", false)

        var computedDisplays = 0
        if (prefStatusbar) computedDisplays = computedDisplays or OriginIslandConstants.DISPLAY_STATUSBAR
        if (prefAod) computedDisplays = computedDisplays or OriginIslandConstants.DISPLAY_AOD
        if (prefLockscreen) computedDisplays = computedDisplays or OriginIslandConstants.DISPLAY_LOCKSCREEN
        if (prefNotification) computedDisplays = computedDisplays or OriginIslandConstants.DISPLAY_NOTIFICATION
        if (prefWidget) computedDisplays = computedDisplays or OriginIslandConstants.DISPLAY_WIDGET
        if (prefVSuggestion) computedDisplays = computedDisplays or OriginIslandConstants.DISPLAY_V_SUGGESTION
        if (computedDisplays == 0) computedDisplays = OriginIslandConstants.DISPLAY_STATUSBAR

        val displays = computedDisplays

        val showNotify = intent.getBooleanExtra("oi_show_notify", globalPrefs.getBoolean("cast_show_notify", true))
        val islandNotify = intent.getBooleanExtra("oi_island_notify", globalPrefs.getBoolean("cast_island_notify", false))
        val forceShow = intent.getBooleanExtra("oi_force_show", globalPrefs.getBoolean("cast_force_show", false))
        val dismissWhenKill = intent.getBooleanExtra("oi_dismiss_when_kill", globalPrefs.getBoolean("cast_dismiss_when_kill", true))
        val keepScreenOn = intent.getBooleanExtra("oi_keep_screen_on", globalPrefs.getBoolean("cast_keep_screen_on", false))
        val sound = intent.getBooleanExtra("oi_sound", globalPrefs.getBoolean("cast_sound", false))
        val islandClick = intent.getIntExtra("oi_island_click", globalPrefs.getInt("cast_island_click_action", 0))

        val disableInvert = intent.getBooleanExtra("oi_disable_invert", globalPrefs.getBoolean("cast_disable_invert", true))
        val prefCardBg = globalPrefs.getInt("cast_card_bg_color", 0)
        val finalCardBgColor = if (cardBgColor != 0) cardBgColor else prefCardBg
        val prefCapsuleBg = globalPrefs.getInt("cast_capsule_bg_color", 0)
        val finalBgColor = if (bgColor != 0) bgColor else prefCapsuleBg

        // Light effect parameters
        val lightEnabled = globalPrefs.getBoolean("cast_light_enabled", false)
        val prefLightColor = globalPrefs.getInt("cast_light_color", 0)
        val prefLightMode = globalPrefs.getInt("cast_light_mode", 0)
        val finalLightColor = if (lightColor != 0) lightColor else if (lightEnabled) prefLightColor else 0
        val finalLightMode = if (lightMode != 0) lightMode else prefLightMode
        ...
        // Auto-dismiss: reset timer for non-live cards to guarantee exact duration
        autoDismissTasks.remove(id)?.let { mainHandler.removeCallbacks(it) }

        if (!isOngoing && effectiveDismissSec > 0) {
            val task = Runnable {
                autoDismissTasks.remove(id)
                cancelNotification(
            ...
            mainHandler.postDelayed(task, effectiveDismissSec * 1000L)
        }
    }
    ...
    private fun cancelPackageCards(intent: Intent) {
        val pkg = intent.getStringExtra("package_name") ?: return
        val matchingIds = cardPackageMap.filter { it.value == pkg }.keys.toList()
