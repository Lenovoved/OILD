...
fun CastTab(context: Context, prefs: SharedPreferences, onRedoSetup: () -> Unit) {
    var castOn by remember { mutableStateOf(prefs.getBoolean("cast_normal_notifications", false) || prefs.getBoolean("cast_notifications", false)) }
    var mediaOn by remember { mutableStateOf(prefs.getBoolean("cast_media_sessions", false)) }
    var messengerOn by remember { mutableStateOf(prefs.getBoolean("cast_messenger_notifications", false)) }
    var navOn by remember { mutableStateOf(prefs.getBoolean("cast_nav_notifications", false)) }
    var ignoreSilent by remember { mutableStateOf(prefs.getBoolean("cast_ignore_silent", false)) }
...
private fun ToggleRow(label: String, checked: Boolean, onChange: (Boolean) -> Unit) {
    Row(Modifier.fillMaxWidth().padding(vertical = 4.dp), verticalAlignment = Alignment.CenterVertically) {
        ...
        OriginOSSwitch(checked = checked, onCheckedChange = onChange)
    }
}
